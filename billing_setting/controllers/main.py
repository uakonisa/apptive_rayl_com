...
class BillingAccounts(http.Controller):
                ...
                try:
                    ...
                    if hasattr(res_token, 'id'):
                        ...
                        if hasattr(res_add_card, 'id'):
                            ...
                            return request.redirect('/my/billing/?message=Successfully Added Card')
                        else:
                    ...
                    return request.redirect("/my/billing/?error=Invalid parameters were supplied to Stripe's API")
                except stripe.error.AuthenticationError as e:
                    # Authentication with Stripe's API failed
                    # (maybe you changed API keys recently)
                    return request.redirect("/my/billing/?error=Authentication with Stripe's API failed")
                except stripe.error.APIConnectionError as e:
                    # Network communication with Stripe failed
                    return request.redirect('/my/billing/?error=Network communication with Stripe failed')
    ...
    def user_details_submit(self, **post):
        if post:
            ...
            try:
                ...
                # Invalid parameters were supplied to Stripe's API
                return request.redirect("/my/user_details/?error=Invalid parameters were supplied to Stripe's API")
            except stripe.error.AuthenticationError as e:
                # Authentication with Stripe's API failed
                # (maybe you changed API keys recently)
                return request.redirect("/my/user_details/?error=Authentication with Stripe's API failed")
            except stripe.error.APIConnectionError as e:
                # Network communication with Stripe failed
                return request.redirect('/my/user_details/?error=Network communication with Stripe failed')
            except stripe.error.StripeError as e:
                # Display a very generic error to the user, and maybe send
                # yourself an email
                return request.redirect('/my/user_details/?error=' + e)
            except Exception as e:
                # Something else happened, completely unrelated to Stripe
                return request.redirect('/my/user_details/?error=' + e)
        else:
    ...
    @http.route(['/my/payout'], type='http', auth="public", website=True)
    def my_payout(self, **post):
        # bank_details = request.env['bank.details'].sudo().search([])
        pay_details = request.env['bank.details'].sudo().search([])
        values = []
        return request.render("billing_setting.payout_setting", {'values': pay_details})
    ...
    def payout_form_submit(self, *args, **post):
        ...
        if post:
            ...
            if post.get('payment_option') == 'electronic_funds_transfer':
                if post.get('bank_account_number') != post.get('confirm_bank_account_number'):
                    raise ValidationError("Bank Account Number do not match; please retype them.")
                else:
                    vals = {'payout_details': payout, }
            else:
                vals = {
                    'payout_details': payout,
                }
        return request.render("billing_setting.payout_form_submit", vals)
    ...
    @http.route('/payout/account', type='http', auth="public", website=True)
    def my_payout_account(self, **kw):
        digits_to_keep = 4
        mask_char = '#'
        user_id = http.request.env.context.get('uid')
        payout_details = request.env['payout.setting'].sudo().search([('create_uid', '=', user_id)])
        # acc_number = payout_details.bank_account_number
        # num_of_digits = sum(map(str.isdigit, acc_number))
        # digits_to_mask = num_of_digits - digits_to_keep
        # masked_string = re.sub('\d', mask_char, acc_number, digits_to_mask)
        # context = request.session.uid
        return request.render("billing_setting.my_payout_account", {'payout_details': payout_details})

    @http.route('/remove/payout_record', type='http', auth="public", website=True)
    ...
    @http.route('/set/default', type='http', auth="public", website=True)
    def set_default_account(self, **post):
        if post:
            ...
            try:
                ...
                # Invalid parameters were supplied to Stripe's API
                return request.redirect("/billing/account/?error=Invalid parameters were supplied to Stripe's API")
            except stripe.error.AuthenticationError as e:
                # Authentication with Stripe's API failed
                # (maybe you changed API keys recently)
                return request.redirect("/billing/account/?error=Authentication with Stripe's API failed")
            except stripe.error.APIConnectionError as e:
                # Network communication with Stripe failed
                return request.redirect('/billing/account/?error=Network communication with Stripe failed')
            except stripe.error.StripeError as e:
                # Display a very generic error to the user, and maybe send
                # yourself an email
                return request.redirect('/billing/account/?error=' + e)
            except Exception as e:
                # Something else happened, completely unrelated to Stripe
                return request.redirect('/billing/account/?error=' + e)
        else:
            return request.redirect('/shop/?error=Please Check your Details and Try again')

    @http.route('/remove/billing_record', type='http', auth="public", website=True)
    def remove_billing_account(self, **post):
        billing_id = post.get('billing_setting_id')
    ...
    @http.route('/set/payout/default', type='http', auth="public", website=True)
    def set_payout_default_account(self, **post):
        payout_id = post.get('payout_setting_id')
        payout_rec = request.env['default.payout.account'].sudo().search([('id', '=', 1)])
        if payout_rec:
            default_obj = request.env['default.payout.account'].sudo().search([('id', '=', 1)]).write({
